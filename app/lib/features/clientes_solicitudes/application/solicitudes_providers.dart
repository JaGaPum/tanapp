...
final solicitudesEstadoFiltroProvider =
    NotifierProvider<SolicitudesEstadoFiltroNotifier, String?>(
      SolicitudesEstadoFiltroNotifier.new,
    );

final solicitudesListProvider =
    FutureProvider.autoDispose<List<SolicitudCliente>>((ref) async {
      final estado = ref.watch(solicitudesEstadoFiltroProvider);
      return ref
          .watch(solicitudesRepositoryProvider)
          .listSolicitudes(estado: estado);
    });

final solicitudDetailProvider = FutureProvider.autoDispose
    .family<SolicitudCliente, String>((ref, id) {
...
final solicitudesPendientesCountProvider = FutureProvider.autoDispose<int>((
  ref,
) async {
  final lista = await ref
      .watch(solicitudesRepositoryProvider)
      .listSolicitudes(estado: 'PENDIENTE');
  return lista.length;
});
