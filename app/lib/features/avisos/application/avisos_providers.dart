...
final misAvisosEnviadosProvider =
    NotifierProvider.autoDispose<
      MisAvisosEnviadosNotifier,
      PaginaResultado<ClienteAviso>
...
class MisAvisosRecibidosNotifier extends PaginatedNotifier<AvisoRecibido> {
  ...
  Future<List<AvisoRecibido>> cargarPagina(int offset, int limit) {
    return ref
        .read(avisosRepositoryProvider)
        .listMisAvisosRecibidos(offset: offset, limit: limit);
  }
}

final misAvisosRecibidosProvider =
    NotifierProvider.autoDispose<
      MisAvisosRecibidosNotifier,
      PaginaResultado<AvisoRecibido>
...
final avisosEstadisticasProvider =
    FutureProvider.autoDispose<List<AvisoEstadistica>>((ref) async {
      final sedes = await ref.watch(misSedesProvider.future);
      return ref
          .watch(avisosRepositoryProvider)
          .listEstadisticasAvisos(sedes.map((s) => s.idClienteSede).toList());
    });
